...
static StreamSharedContext* buildLocalStreamContext(Stream* streamNode, PlannedStmt* pstmt)
{
    ...
    for (int i = 0; i < consumerNum; i++) {
        subContext[i] = (StreamSharedSubContext *)CacheLineAlign(
            ((StreamSharedSubContext *)palloc0(sizeof(StreamSharedSubContext) * producerNum + CacheLineSize)));
        for (int j = 0; j < producerNum; j++) {
            subContext[i][j].batchManager = NULL;
            subContext[i][j].sendBatch = NULL;
            subContext[i][j].recvBatch = NULL;
            subContext[i][j].status = DATA_EMPTY;
            subContext[i][j].is_connect_end = false;
            subContext[i][j].recv_first_batch = true;
...
static void InitStream(StreamFlowCtl* ctl, StreamTransType transType)
{
    ...
    if (streamNode->is_dummy == false) {
        for (i = 0; i < streamNode->smpDesc.producerDop; i++) {
            ...
            producer->setCopyMode(u_sess->attr.attr_sql.query_dop_mode);

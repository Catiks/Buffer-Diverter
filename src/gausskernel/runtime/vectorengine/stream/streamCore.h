...
typedef struct StreamSharedSubContext {
    class BatchManager* batchManager;
    BatchVector* sendBatch;
    BatchVector* recvBatch;
    TupleVector* sharedTuples;
    StringInfo messages;
    DataStatus status;
    bool is_connect_end;
    bool recv_first_batch;
    struct hash_entry* quota_entrys;
} StreamSharedSubContext __attribute__((aligned(CacheLineSize)));
...
// switch batch for smp
class BatchManager : public BaseObject {

public:
    ~BatchManager() {}

    static BatchManager* CreateBatchManager(TupleDesc desc, Bitmapset* AssignColumn = NULL, bool needAlign = true, bool initStream = false);

    BatchVector* GetCurrentBatch() { return _batchArray[_batchPointor]; }

    uint8 GetPointor() { return _batchPointor; }

    void SwitchBatch();

    VecExprState** CreateProjVectorExpr(List *targetList, TupleTableSlot* slot, PlanState *parent, TupleDesc inputDesc);

private:
    BatchManager(MemoryContext cxt, TupleDesc desc, uint8 switchNum, Bitmapset* AssignColumn = NULL, bool needAlign = true);

    void Init();

private:
    MemoryContext _cxt;
    BatchVector** _batchArray;
    uint8 _switchNum;
    uint8 _batchPointor;
    bool _needSwitch;
};

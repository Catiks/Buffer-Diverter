...
bool gs_is_databuff_empty(StreamSharedContext* sharedContext, int nthChannel)
{
    if (sharedContext->vectorized) {
        BatchVector* batch = sharedContext->subContext[nthChannel][u_sess->stream_cxt.smp_id].batchManager->GetCurrentBatch();
        if (batch->Rows() == 0) {
            return true;
        }
    } else {
...
void gs_memory_send_batch(BatchVector* batchsrc, StreamSharedSubContext* subContext, struct hash_entry* entry)
{
    ...
    if (subContext->batchManager) {
        BatchVector* batchdst = subContext->batchManager->GetCurrentBatch();
        batchdst->DeepCopy(batchsrc);
    } else 
        /* Copy pointor to shared context. */
        subContext->sendBatch = batchsrc;
...
extern void gs_memory_send_batch_distribution(BatchVector *batchsrc, StreamSharedSubContext *subContext,
                                              struct hash_entry *entry, StreamLocator *loc)
{
    ...
    for (;;) {
        ...
        if ((subContext->status == DATA_EMPTY
#ifdef __aarch64__
            // todo zwh: how to handle batch row check in arm64
             && gs_is_databuff_empty(sharedContext, nthChannel)
#endif
             ) ||
            subContext->status == DATA_PREPARE) {
    ...
    for (; loc->pointer < loc->size; loc->pointer++) {
        for (int i = 0; i < batchsrc->Cols(); i++) {
            CVector* vectorsrc = batchsrc->GetColumn(i);
            CVector* vectordst = batchdst->GetColumn(i);
            Datum data = vectorsrc->At(loc->locator[loc->pointer]);
            Size size = vectorsrc->AtSize(loc->locator[loc->pointer]);
            int pos = loc->locator[loc->pointer];
            vectordst->BatchPushBack(vectorsrc, pos, pos + 1);
        }
        batchdst->SetRows(batchdst->Rows() + 1);
        if (ColumnVectorSize == batchdst->Rows()) {
            loc->pointer++;
            ready_to_send = true;
            break;
        }
...
bool gs_consume_memory_data(StreamState* node, int loc)
{
    ...
    if (sharedContext->vectorized) {
        BatchVector* batch;
        if (subContext->batchManager == NULL) {
            batch = subContext->recvBatch = subContext->sendBatch;
        } else {
            batch = subContext->batchManager->GetCurrentBatch();
            subContext->batchManager->SwitchBatch();
            subContext->batchManager->GetCurrentBatch()->Reset();
        }
        ((VecStreamState*)node)->m_CurrentBatch = (VectorBatch*)batch;
    } else {

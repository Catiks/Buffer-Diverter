...
/*
 * @Description: Set copy mode for local data production
 *
 * @param[IN] mode: copy mode
 * @return: void
 */
void StreamProducer::setCopyMode(int mode)
{
    m_copyMode = mode;
}
...
void StreamProducer::netInit()
{
    ...
    if (STREAM_IS_LOCAL_NODE(m_streamNode->smpDesc.distriType)) {
        ...
        m_batchLocator = (StreamLocator*)palloc0(sizeof(StreamLocator) * m_parallel_desc.consumerDop);
        return;
    }
 ...
 * @return: void
 */
void StreamProducer::localBroadCastStream(BatchVector* batchSrc)
{
    for (int i = 0; i < m_connNum; i++) {
        sendByMemoryBatch(batchSrc, i);
    }
}
 ...
 * @return: void
 */
void StreamProducer::localRedistributeStream(BatchVector* batch)
{
    Assert(m_sharedContext != NULL);
    ...
    while(true) {
        ...
        for (int i = 0; i < m_parallel_desc.consumerDop; i++) {
            ...
            if (m_batchLocator[i].size > 0) {
                sendByMemoryBatchDistribution(batch, i, &m_batchLocator[i]);
                if (m_batchLocator[i].pointer == m_batchLocator[i].size) {
                    m_batchLocator[i].size = 0;
                    m_batchLocator[i].pointer = 0;
                } else {
                    skip = false;
                }
            }
 ...
 * @return: void
 */
void StreamProducer::localRoundRobinStream(BatchVector* batch)
{
    sendByMemoryBatch(batch, m_roundRobinIdx);
 ...
 *
 * @param[IN] batch:  batch vector
 * @return: void
 */
template<int keyNum>
void StreamProducer::redistributeBatchChannel(BatchVector* batch)
{
    /*
    ...
    */
    CVector* pDistributeVec = NULL;
    uint64 hashValue[ColumnVectorSize] = {0};
    bool isNull[ColumnVectorSize] = {true};
    Datum data;
    ...
    if (keyNum >= 1) {
        pDistributeVec = batch->GetColumn(m_distributeIdx[0]);
        for (uint32 i = 0; i < batch->Rows(); i++) {
            // todo zwh: support null
            data = pDistributeVec->At(i);
    ...
    if (keyNum >= 2) {
        pDistributeVec = batch->GetColumn(m_distributeIdx[1]);
        for (uint32 i = 0; i < batch->Rows(); i++) {
            data = pDistributeVec->At(i);
            if (!isNull[i]) {
    ...
    if (keyNum == 3) {
        ...
        for (int j = 2; j < redistributeKeyNum; j++) {
            pDistributeVec = batch->GetColumn(m_distributeIdx[j]);
    ...
    for (uint32 i = 0; i < batch->Rows(); i++) {
        int consumer_id = ChannelLocalizer(hashValue[i], dop);
        m_batchLocator[consumer_id].locator[m_batchLocator[consumer_id].size] = i;
        m_batchLocator[consumer_id].size++;
    }
}

template<int distrType>
void StreamProducer::redistributeBatchChannelForSlice(BatchVector* batch)
{
    if (distrType == REMOTE_DIRECT_DISTRIBUTE) {
        return;
    }

    // todo zwh:  use new batch
    VectorBatch* old_batch = (VectorBatch*)batch;
    int keyNum;
    ...
    for (int i = 0; i < old_batch->m_rows; i++) {
        allIsNull = true;
        hashValue = 0;

        for (int j = 0; j < keyNum; j++) {
            pDistributeVec = &old_batch->m_arr[m_distributeIdx[j]];
...
void StreamProducer::DispatchBatchRedistrFunctionByRedisType()
{
    switch (m_parallel_desc.distriType) {
        ...
        case LOCAL_DISTRIBUTE:
            m_channelCalVecFun = &StreamProducer::redistributeBatchChannel<len>;
            break;
...
bool StreamProducer::isDistributeStream()
{
    return (STREAM_REDISTRIBUTE == m_streamType && LOCAL_DISTRIBUTE == m_parallel_desc.distriType);
}
...
int StreamProducer::ChannelLocalizer(ScalarValue hashValue, int dop)
{
    Assert(m_disQuickLocator != NULL);
    int nodeIdx = 0;
    int threadIdx = ThreadLocalizer(hashValue, dop);
    return m_disQuickLocator[nodeIdx][threadIdx];
}
...
void StreamProducer::sendByMemoryBatch(BatchVector* batchSrc, int nthChannel)
{
    if (likely(m_sharedContextInit)) {
        gs_memory_send_batch(batchSrc, &(m_sharedContext->subContext[nthChannel][u_sess->stream_cxt.smp_id]),
                             m_sharedContext->poll_entrys[nthChannel]);
...
void StreamProducer::sendByMemoryBatchDistribution(BatchVector* batchSrc, int nthChannel, StreamLocator *loc)
{
    if (likely(m_sharedContextInit)) {
        gs_memory_send_batch_distribution(batchSrc,
                                          &(m_sharedContext->subContext[nthChannel][u_sess->stream_cxt.smp_id]),
                                          m_sharedContext->poll_entrys[nthChannel], loc);
...
void StreamProducer::initSharedContext()
{
    ...
    if (m_sharedContext->vectorized) {
        ...
        if (isDistributeStream() || m_copyMode == QUERY_DOP_COPYONCE) {
            for (int i = 0; i < m_connNum; i++) {
                m_sharedContext->subContext[i][u_sess->stream_cxt.smp_id].batchManager =
                    BatchManager::CreateBatchManager(m_desc, NULL, true, true);
            }
        }
    } else {
        /* Init tuples. */
...
void StreamProducer::redistributeStream(BatchVector* batch)
{
...
void StreamProducer::broadCastStream(BatchVector* batch)
{
    DISTRIBUTED_FEATURE_NOT_SUPPORTED();
    return;
}
...
void StreamProducer::broadCastStreamCompress(BatchVector* batch)
{
    DISTRIBUTED_FEATURE_NOT_SUPPORTED();
    return;
}
...
void StreamProducer::roundRobinBatch(BatchVector* batch)
{
    DISTRIBUTED_FEATURE_NOT_SUPPORTED();
    return;
}
...
void StreamProducer::hybridStream(BatchVector* batch, DestReceiver* self)
{
    DISTRIBUTED_FEATURE_NOT_SUPPORTED();
    return;
}

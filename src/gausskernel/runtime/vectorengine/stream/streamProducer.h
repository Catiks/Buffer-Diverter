...
public:
    ...
    void broadCastStream(BatchVector* batch);

    /* Broadcast the batch with compress. */
    void broadCastStreamCompress(BatchVector* batch);

    /* Local broadcast the batch throug memory. */
    void localBroadCastStream(BatchVector* batch);

    /* Redistribute the batch. */
    void redistributeStream(BatchVector* batch);
    ...
    void serializeStream(BatchVector* batch, int index);

    /* Local redistribute the batch through memory. */
    void localRedistributeStream(BatchVector* batch);

    /* Send tuple batch Roundrobin. */
    template <BatchCompressType ctype>
    void roundRobinBatch(BatchVector* batch);

    /* Local roundrobin the batch through memory. */
    void localRoundRobinStream(BatchVector* batch);

    /* Hybrid Stream Type. */
    void hybridStream(BatchVector* batch, DestReceiver* self);
    ...
    /* Copy the batch to shared memory. */
    void sendByMemoryBatch(BatchVector* batchSrc, int nthChannel);

    /* Copy the batch distribution to shared memory. */
    void sendByMemoryBatchDistribution(BatchVector* batchSrc, int nthChannel, StreamLocator *loc);
...
public:
    ...
    /* Get copy mode for locl data production. */
    FORCE_INLINE int getCopyMode() { return m_copyMode; }
    ...
    /* Set copy mode for local data production. */
    void setCopyMode(int mode);
...
private:
    ...
    void (StreamProducer::*m_channelCalVecFun)(BatchVector* batch);
    ...
    template<int keyNum>
    void redistributeBatchChannel(BatchVector* batch);
    ...
    inline int ChannelLocalizer(ScalarValue hashValue, int Dop);
    ...
    template<int distrType>
    void redistributeBatchChannelForSlice(BatchVector* batch);
...
private:
    ...
    /* Copy mode of stream production. */
    int m_copyMode;

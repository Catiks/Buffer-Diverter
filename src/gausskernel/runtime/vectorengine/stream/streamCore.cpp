...
void StreamNodeGroup::ReleaseStreamGroup(bool resetSession, StreamObjStatus status)
{
    if (u_sess->stream_cxt.global_obj != NULL) {
        ...
        if (u_sess->attr.attr_sql.query_dop_debug) {
            ereport(LOG, (errmsg("SMP in waiting quit (StreamNodeGroup::ReleaseStreamGroup)")));
            sleep(20);
        }
...
BatchManager* BatchManager::CreateBatchManager(TupleDesc desc, Bitmapset* AssignColumn, bool needAlign, bool initStream)
{
    MemoryContext cxt;
    uint8 switchNum;
    if (StreamThreadAmI()) {
        cxt = u_sess->stream_cxt.producer_obj->getSharedContext()->localStreamMemoryCtx;
        if (initStream) {
            if (u_sess->stream_cxt.producer_obj->isDistributeStream() ||
                u_sess->stream_cxt.producer_obj->getCopyMode() == QUERY_DOP_COPYONCE) {
                switchNum = 2;
            } else
                // no need to create batch manager
                Assert(false);
        } else {
            if (u_sess->stream_cxt.producer_obj->isDistributeStream() ||
                u_sess->stream_cxt.producer_obj->getCopyMode() == QUERY_DOP_COPYONCE) {
    ...
    BatchManager* manager = New(cxt)BatchManager(cxt, desc, switchNum, AssignColumn, needAlign);
    manager->Init();
    return manager;
}

void BatchManager::SwitchBatch()
{
    if (_needSwitch) {
        _batchPointor = (_batchPointor + 1) % _switchNum;
    }
}

VecExprState** BatchManager::CreateProjVectorExpr(List *targetList, TupleTableSlot* slot, PlanState *parent, TupleDesc inputDesc)
{
    VecExprState** state;
    MemoryContext oldCxt = MemoryContextSwitchTo(_cxt);
    state = (VecExprState**)palloc(sizeof(VecExprState*) * _switchNum);
    for (int i = 0; i < _switchNum; i++) {
	    state[i] = ExecInitProjVectorExpr(targetList, slot, parent, inputDesc);
    }
    MemoryContextSwitchTo(oldCxt);
    return state;
}

BatchManager::BatchManager(MemoryContext cxt, TupleDesc desc, uint8 switchNum, Bitmapset* AssignColumn, bool needAlign)
{
    _batchPointor = 0;
    _cxt = cxt;
    _switchNum = switchNum;
    _needSwitch = (switchNum > 1) ? true : false;
    MemoryContext oldCxt = MemoryContextSwitchTo(_cxt);
    _batchArray = (BatchVector**)palloc(sizeof(BatchVector*) * _switchNum);
    for (int i = 0; i < _switchNum; i++) {
        _batchArray[i] = New(_cxt) BatchVector(_cxt, desc, AssignColumn, needAlign);
    }
    MemoryContextSwitchTo(oldCxt);
}

...
public:
    ...
    virtual Datum At(int idx) = 0;
    virtual uint32 AtSize(int idx) = 0;
    virtual bool AtSame(int idx, void* value, uint32 size = 0) = 0;
...
public:
    ...
    FORCE_INLINE void* Alloc(MemoryContext cxt, uint32 size) {
        if (IsUnalloc()) {
...
public:
    ColumnVector(MemoryContext cxt) : _len(0), _nulllen(0), _bitmap(NULL), _data(NULL), _cxt(cxt) {}
    ...
    FORCE_INLINE void Clear() override { Assert(false); }
    ...
    FORCE_INLINE bool AtSame(int idx, void* value, uint32 size = 0) override;
...
public:
    ColumnVectorConst(MemoryContext cxt) : _len(0), _size(0), _data(0), _cxt(cxt) {}
    ...
    void VectorPack(CVector* src, VecQualResult* QualResult, int32* datapos) override { Assert(false); return; }

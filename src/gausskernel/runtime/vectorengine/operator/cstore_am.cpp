...
void CStore::BindingFp(CStoreScanState* state)
{
    ...
    volatile uint32** parallelCUID = NULL;

    if (proj->pi_maxOrmin) {
        Assert(list_length(proj->pi_maxOrmin) == list_length(proj->pi_accessedVar));
...
void CStore::InitFillVecEnv(CStoreScanState* state)
{
    ...
    ProjectionInfo* proj = state->ps.ps_ProjInfo;
    if (proj->pi_accessedVar != NIL) {
        List* pColList = proj->pi_accessedVar;
        ...
        foreach (cell, proj->pi_lateAccessedVarNumbers) {
            int colId = lfirst_int(cell) - 1;
            for (i = 0; i < m_colNum; ++i) {
                if (colId == m_colId[i]) {
                    m_lateRead[i] = true;
        ...
        pinSlot = (CStoreCUPinInfo*)palloc(sizeof(CStoreCUPinInfo) * m_colNum);
        ConstVectors = (ConstCUVectors*)palloc(sizeof(ConstCUVectors) * m_colNum);

        for (int i = 0;i < m_colNum;i++) {
            pinSlot[i].slot = CACHE_BLOCK_INVALID_IDX;
            pinSlot[i].cuptr = NULL;      
        }

        i = 0;
        foreach (cell, pColList) {
            int colId = lfirst_int(cell) - 1;
            ConstVectors[i].vector = AllocColumnVectorByType(CurrentMemoryContext, 
                m_relation->rd_att->attrs[colId].atttypid);
            ConstVectors[i].vector->Init();
            ConstVectors[i].init = false;
            i++;
        }
        BindingFp(state);
        CreateParallelCUID();
...
void CStore::Destroy()
{
    if (m_relation != NULL) {
        ...
        for (int i = 0; i < m_colNum; i++) {
            if (IsValidCacheSlotID(pinSlot[i].slot)) {
                Assert(pinSlot[i].cuptr);
                CUCache->UnPinDataBlock(pinSlot[i].slot);
                pinSlot[i].slot = CACHE_BLOCK_INVALID_IDX;
                pinSlot[i].cuptr = NULL;
            }
        }
...
void CStore::CStoreScan(CStoreScanState* state, BatchVector* batchvector)
{
    RefreshCursor();
    ...
    FillVecBatch<false>(batchvector);

    m_CursorRows = batchvector->Rows();
}
...
void CStore::LoadCUDescIfNeed()
{
    ...
    ADIO_RUN()
    {
        prefetch_control = true;
    }
    ADIO_ELSE()
    {
        // if disable adio, use parallel cuid in stream
        if (m_parallelCUID) {
            m_currentCUID = 0;
            prefetch_control = true;
        } else {
            prefetch_control = false;
        }
    }
    ADIO_END();
    ...
    do {
        ...
        for (int i = 0; i < m_colNum; ++i) {
            ...
            found =
                LoadCUDesc(m_colId[i], m_CUDescInfo[i], prefetch_control, m_snapshot);
        }
...
void CStore::CreateParallelCUID()
{
    StreamSharedContext* sharedContext = NULL;
    if (StreamTopConsumerAmI() && u_sess->attr.attr_sql.enable_stream_cuid) {
        Assert(u_sess->stream_cxt.global_obj != NULL);
        sharedContext = u_sess->stream_cxt.global_obj->getCurrentContext();
        // If sharedContext is null, that means scan will be executed by top consumer or a stream producer with dop = 1.
        // So there is not necessary to create shared cuid.
        if (sharedContext) {
            MemoryContext oldCxt = MemoryContextSwitchTo(sharedContext->localStreamMemoryCtx);
            sharedContext->parallel_cuid = (volatile uint32 *)palloc(sizeof(volatile uint32));
            pg_atomic_init_u32(sharedContext->parallel_cuid, m_startCUID - 1);
            MemoryContextSwitchTo(oldCxt);
        }
    } else if (StreamThreadAmI() && u_sess->stream_cxt.producer_dop > 1) {
        Assert(u_sess->stream_cxt.producer_obj != NULL);
        sharedContext = u_sess->stream_cxt.producer_obj->getSharedContext();
        m_parallelCUID = sharedContext->parallel_cuid;
    }
}
    ...
    _in_ int col, __inout LoadCUDescCtl* loadCUDescInfoPtr, _in_ bool prefetch_control, _in_ Snapshot snapShot)
{
    ...
    uint32 expected_cuid = 0;

    if (m_currentCUID) {
        loadCUDescInfoPtr->nextCUID = m_currentCUID;
    } else if (m_parallelCUID) {
        expected_cuid = pg_atomic_read_u32(m_parallelCUID);
        loadCUDescInfoPtr->nextCUID = expected_cuid + 1;
    }
    /*
    ...
    while ((tup = systable_getnext_ordered(cudesc_scan, ForwardScanDirection)) != NULL) {
        ...
        ADIO_ELSE()
        {
            ...
            if (m_parallelCUID) {
                /* Use parallel CU id. */
                if (m_currentCUID == 0) {
                    if (cu_id <= expected_cuid)
                        continue;
                    if (!pg_atomic_compare_exchange_u32(m_parallelCUID, &expected_cuid, cu_id)) {
                        continue;
                    }
                    m_currentCUID = cu_id;
                }
...
void CStore::FillVecBatch(BatchVector* batchvector)
{
    int i, colIdx;
    ...
    for (i = 0; i < m_colNum; ++i) {
        ...
        if (likely(colIdx >= 0)) {
            if (LateRead) {
                if (IsLateRead(i)) {
                    CUDesc* cuDescPtr = m_CUDescInfo[i]->cuDescArray + this->m_cuDescIdx;
                    GetCUDeleteMaskIfNeed(cuDescPtr->cu_id, m_snapshot);
                    (this->*m_colFillFunArrary[i].colFillFun[(int)m_hasDeadRow])(i, cuDescPtr, batchvector->GetColumn(colIdx));
                    batchvector->SetRows(batchvector->GetColumn(colIdx)->Size());
                }
            }
            else {
                if (!IsLateRead(i)) {
                    CUDesc* cuDescPtr = m_CUDescInfo[i]->cuDescArray + m_CUDescIdx[m_cursor];
                    GetCUDeleteMaskIfNeed(cuDescPtr->cu_id, m_snapshot);
                    this->m_cuDescIdx = m_CUDescIdx[m_cursor];
                    (this->*m_colFillFunArrary[i].colFillFun[(int)m_hasDeadRow])(i, cuDescPtr, batchvector->GetColumn(colIdx));
                    batchvector->SetRows(batchvector->GetColumn(colIdx)->Size());
                }
            }
...
template void CStore::FillVecBatch<true>(BatchVector* batchvector);
template void CStore::FillVecBatch<false>(BatchVector* batchvector);

// Fill vector of column
template <bool hasDeadRow, int attlen>
int CStore::FillVector(_in_ int seq, _in_ CUDesc* cuDescPtr, _out_ CVector* vector)
{
    ...
    if (cuDescPtr->IsSameValCU()) {
        bool same = true;
        if (ConstVectors[seq].init && ConstVectors[seq].cu_id != cuDescPtr->cu_id) {
            void *addr;
            int len;
            if (attlen > 0) {
                addr = (void*)(cuDescPtr->cu_min);
                len = attlen;
            }
            else if (attlen < 0) {
                addr = (void*)(cuDescPtr->cu_min + 1);
                len = (int)cuDescPtr->cu_min[0];
            }
            same = ConstVectors[seq].vector->AtSame(0, addr, len);
            if (same)
                ConstVectors[seq].cu_id = cuDescPtr->cu_id;
        }

        if (!same || !ConstVectors[seq].init) {
            if (attlen > 0) {
                ConstVectors[seq].vector->InitByValue(cuDescPtr->cu_min, ColumnVectorSize);
            }
            else {
                Size varLen;
                ...
                if (attlen == -1) {
                    varLen = len + VARHDRSZ_SHORT;
                    SET_VARSIZE_SHORT(tmpStr, varLen);
                    errno_t rc = memcpy_s(VARDATA_ANY(tmpStr), sizeof(tmpStr) - VARHDRSZ_SHORT, DatumGetPointer(cuMin), len);
                    securec_check(rc, "", "");
                    cuMin = PointerGetDatum(tmpStr);
                }
                ConstVectors[seq].vector->InitByValue(cuDescPtr->cu_min, ColumnVectorSize, varLen);
            }
            ConstVectors[seq].cu_id = cuDescPtr->cu_id;
            ConstVectors[seq].init = true;
        }

        pos = leftRows > ColumnVectorSize ? ColumnVectorSize : leftRows;
        vector->ShallowCopy(ConstVectors[seq].vector);
        vector->Resize(pos);
        return pos;
    }
    ...
    if (IsValidCacheSlotID(pinSlot[seq].slot) && pinSlot[seq].cuptr) {
        cuPtr = pinSlot[seq].cuptr;
    }
    else {
        if (unlikely(IsValidCacheSlotID(pinSlot[seq].slot)))
            CUCache->UnPinDataBlock(pinSlot[seq].slot);
        pinSlot[seq].slot = CACHE_BLOCK_INVALID_IDX;
        pinSlot[seq].cuptr = NULL;

        CSTORESCAN_TRACE_START(GET_CU_DATA);
        cuPtr = this->GetCUData(cuDescPtr, colIdx, attlen, pinSlot[seq].slot);
        CSTORESCAN_TRACE_END(GET_CU_DATA);

        if (likely(IsValidCacheSlotID(pinSlot[seq].slot))) {
            pinSlot[seq].cuptr = cuPtr;
        }
        else {
            pinSlot[seq].slot = CACHE_BLOCK_INVALID_IDX;
            pinSlot[seq].cuptr = NULL;
    ...
    // step 5: CUToVector
    pos = cuPtr->ToVector<attlen, hasDeadRow>(vector, leftRows, this->m_rowCursorInCU, deadRows, this->m_cuDelMask);
...
void CStore::CheckConsistenceOfCUData(CUDesc* cuDescPtr, CU* cu, AttrNumber col) const
{
    ...
    if (cu->m_offsetSize > 0) {
        ...
        uint32 count = cuDescPtr->row_count / ColumnVectorSize;
        uint32 left_count = cuDescPtr->row_count % ColumnVectorSize;
        if ((cu->m_offsetSize / (int)sizeof(int32)) != (cuDescPtr->row_count + count + !!left_count)) {
            ereport(defence_errlevel(),
                    (errcode(ERRCODE_INTERNAL_ERROR),
                     errmsg("row_count mismatch between cached CU data and CUDesc, CUDesc's row_count %d, CU's "
                            "row_count %d",
                            (cuDescPtr->row_count + count + !!left_count),
                            (cu->m_offsetSize / (int)sizeof(int32))),
                     errdetail("relation info: name \"%s\", namespace id %u, id %u, relfilenode %u/%u/%u",
                               RelationGetRelationName(m_relation),
...
CStoreScanDesc CStoreBeginScan(Relation relation, int colNum, int16* colIdx, Snapshot snapshot, bool scanDelta)
{
    ...
    projInfo->pi_accessedVar = accessAttrList;
...
void CStoreEndScan(CStoreScanDesc cstoreScanState)
{
    ...
    if (projInfo->pi_accessedVar) {
        list_free(projInfo->pi_accessedVar);
    }

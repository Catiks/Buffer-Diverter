...
{
	elog(INFO, "should use new function!!!!");
	return NULL;
}

template <SimpleOp sop, bool append, typename DataType, typename ResultType>
void vint_sop_const_left(VECTOR_FUNCTION_ARGS)
{
    DataType data1 = *(reinterpret_cast<DataType*>(fcinfo->args[0]));
    DataType* data2 = *(reinterpret_cast<DataType**>(fcinfo->args[1]));
    ResultType* data3 = *(reinterpret_cast<ResultType**>(fcinfo->args[2]));

	if (append) {
    	for (uint32 i = 0;i < fcinfo->rows; ++i) {
			data3[i] |= eval_simple_op<sop, DataType>(data1, data2[i]);
		}
	}
	else {
		for (uint32 i = 0;i < fcinfo->rows; ++i) {
...
template <SimpleOp sop, bool append, typename DataType, typename ResultType>
void vint_sop_const_right(VECTOR_FUNCTION_ARGS)
{
    DataType* data1 = *(reinterpret_cast<DataType**>(fcinfo->args[0]));
    DataType data2 = *(reinterpret_cast<DataType*>(fcinfo->args[1]));
    ResultType* data3 = *(reinterpret_cast<ResultType**>(fcinfo->args[2]));
	...
	if (append) {
    	for (uint32 i = 0;i < fcinfo->rows; ++i) {
			data3[i] |= eval_simple_op<sop, DataType>(data1[i], data2);
		}
	}
	else {
    	for (uint32 i = 0;i < fcinfo->rows; ++i) {
...
template <SimpleOp sop, bool append, typename Lefttype, typename Righttype, typename ResultType>
void vint_sop_left(VECTOR_FUNCTION_ARGS)
{
    Lefttype* data1 = *(reinterpret_cast<Lefttype**>(fcinfo->args[0]));
    Righttype* data2 = *(reinterpret_cast<Righttype**>(fcinfo->args[1]));
    ResultType* data3 = *(reinterpret_cast<ResultType**>(fcinfo->args[2]));

	if (append) {
    	for (uint32 i = 0;i < fcinfo->rows; ++i) {
			data3[i] |= eval_simple_op<sop, Lefttype>(data1[i], (Lefttype)data2[i]);
		}
	}
	else {
    	for (uint32 i = 0;i < fcinfo->rows; ++i) {
...
template <SimpleOp sop, bool append, typename Lefttype, typename Righttype, typename ResultType>
void vint_sop_right(VECTOR_FUNCTION_ARGS)
{
    Lefttype* data1 = *(reinterpret_cast<Lefttype**>(fcinfo->args[0]));
    Righttype* data2 = *(reinterpret_cast<Righttype**>(fcinfo->args[1]));
    ResultType* data3 = *(reinterpret_cast<ResultType**>(fcinfo->args[2]));
	...
	if (append) {
    	for (uint32 i = 0;i < fcinfo->rows; ++i) {
			data3[i] |= eval_simple_op<sop, Righttype>((Righttype)data1[i], data2[i]);
		}
	}
	else {
		for (uint32 i = 0;i < fcinfo->rows; ++i) {
...
inline void vector_int_cmp(VECTOR_FUNCTION_ARGS)
{
    DataType arg1 = DatumGetInt64(PointerGetDatum(fcinfo->args[0]));
    DataType arg2 = DatumGetInt64(PointerGetDatum(fcinfo->args[2]));
	int32 result = 0;

#ifdef HAVE_INT64_TIMESTAMP
    result = (arg1 < arg2) ? -1 : ((arg1 > arg2) ? 1 : 0);
#else
	// unsupported float8 timestamp
	Assert(true);
#endif

	fcinfo->rows = (uint32)result;
}

...
{
    uint32 count = rows / ColumnVectorSize;
    uint32 left_count = rows % ColumnVectorSize;
    uint32 vcount;
    uint32 base, offset, start, end;
    uint32 i, j;
    ...
    vcount = count + !!left_count;
    Assert(NULL == this->m_offset);
    this->m_offset = (int32*)CStoreMemAlloc::Palloc(sizeof(int32) * (rows + vcount), !this->m_inCUCache);
    this->m_offsetSize = sizeof(int32) * (rows + vcount);
    this->m_offset_ps = (int32*)CStoreMemAlloc::Palloc(sizeof(int32) * (vcount + 1), !this->m_inCUCache);
    this->m_offset_ps[0] = 0;
    ...
    if ((m_infoMode & CU_IntLikeCompressed)
        ...
        for (i = 0; i < count; ++i) {
            start = (ColumnVectorSize + 1) * i + 1;
            end = start + ColumnVectorSize;
            this->m_offset[start - 1] = 0;
            for (j = start; j < end; ++j) {
                this->m_offset[j] = j * NUMERIC_64SZ;
            }
            this->m_offset_ps[i + 1] = this->m_offset_ps[i] + this->m_offset[j - 1];
        }
        if (left_count) {
            start = count * (ColumnVectorSize + 1) + 1;
            end = start + left_count;
            this->m_offset[start - 1] = 0;
            for (j = start; j < end; ++j) {
                this->m_offset[j] = j * NUMERIC_64SZ;
            }
            this->m_offset_ps[count + 1] = this->m_offset_ps[count] + this->m_offset[j - 1];
        }
        return;
    }
    ...
    if (-1 == this->m_eachValSize) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            this->m_offset[start - 1] = 0;
            for (j = start; j < end; ++j) {
                this->m_offset[j] = this->m_offset[j - 1] + VARSIZE_ANY(this->m_srcData + offset);
                offset = base + this->m_offset[j];
            }
            this->m_offset_ps[i + 1] = this->m_offset_ps[i] + this->m_offset[j - 1];
        }
        if (left_count) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            this->m_offset[start - 1] = 0;
            for (j = start; j < end; ++j) {
                this->m_offset[j] = this->m_offset[j - 1] + (strlen(this->m_srcData + offset) + 1);
                offset = base + this->m_offset[j];
            }
            this->m_offset_ps[i + 1] = this->m_offset_ps[i] + this->m_offset[j - 1];
        }
        ...
        for (i = 0; i < count; ++i) {
            ...
            this->m_offset[start - 1] = 0;
            for (j = start; j < end; ++j) {
                this->m_offset[j] = this->m_offset[j - 1] + this->m_eachValSize;
                offset = base + this->m_offset[j];
            }
            this->m_offset_ps[i + 1] = this->m_offset_ps[i] + this->m_offset[j - 1];
        }
...
void CU::RebuildCacheForStr(int rows)
{
    uint32 count = rows / ColumnVectorSize;
    uint32 left_count = rows % ColumnVectorSize;
    uint32 vcount;
    uint32 offset, start, end;
    uint32 i, j;
    char* dstData;
    uint32 dstSize;
    uint32 dstOffset;
    ...
    this->m_offset_ps = (int32*)CStoreMemAlloc::Palloc(sizeof(int32) * (vcount + 1), !this->m_inCUCache);
    this->m_offset_ps[0] = 0;
    dstSize = m_srcBufSize - (sizeof(uint32) * rows);
    dstData = (char*)CStoreMemAlloc::Palloc(dstSize, !this->m_inCUCache);
    ...
    if (-1 == this->m_eachValSize) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            this->m_offset[start - 1] = 0;
            for (j = start; j < end; ++j) {
                valesize = VARSIZE(this->m_srcData + offset);
                Assert((valesize - VARHDRSZ) > 0);
                this->m_offset[j] = this->m_offset[j - 1] + (valesize - VARHDRSZ);
                memcpy(dstData + dstOffset, this->m_srcData + offset + VARHDRSZ, valesize - VARHDRSZ);
                offset += valesize;
                dstOffset += (valesize - VARHDRSZ);
            }
            this->m_offset_ps[i + 1] = this->m_offset_ps[i] + this->m_offset[j - 1];
        }
        if (left_count) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            this->m_offset[start - 1] = 0;
            for (j = start; j < end; ++j) {
                valesize = strlen(this->m_srcData + offset) + 1;
                Assert((valesize - VARHDRSZ - 1) > 0);
                this->m_offset[j] = this->m_offset[j - 1] + (valesize - VARHDRSZ - 1);
                memcpy(dstData + dstOffset, this->m_srcData + offset + VARHDRSZ, valesize - VARHDRSZ - 1);
                offset += valesize;
                dstOffset += (valesize - VARHDRSZ - 1);
            }
            this->m_offset_ps[i + 1] = this->m_offset_ps[i] + this->m_offset[j - 1];
        }
        if (left_count) {
    ...
    Assert(this->m_srcData == this->m_srcBuf);
    CStoreMemAlloc::Pfree(this->m_srcBuf, !m_inCUCache);
    this->m_srcBuf = dstData;
    this->m_srcData = dstData;
    this->m_nulls = NULL;
    this->m_srcBufSize = dstSize;
}
...
int CU::ToVectorT(_out_ CVector* vector, _in_ int leftRows, _in_ int rowCursorInCU, _out_ int& deadRows, _in_ uint8* cuDelMask)
{   
    int rows = leftRows > ColumnVectorSize ? ColumnVectorSize : leftRows;
    uint32 pos;
    ...
    if (hasNull) {
        ...
        if (hasDeadRow) {
            ...
            if (attlen > 0) {
                pos = rowCursorInCU * attlen;
                vector->InitByAddr((void*)(m_srcData + pos), rows);
            }
            else if (attlen < 0) {
                Assert((rowCursorInCU % ColumnVectorSize) == 0);
                uint32 count = rowCursorInCU / ColumnVectorSize;
                int32* offset = m_offset + rowCursorInCU + count + 1;
                pos = m_offset_ps[count];
                vector->InitByAddr((void*)(m_srcData + pos), rows, 0, (uint32*)offset, m_offset_ps[count + 1] - m_offset_ps[count]);
            }
        }
    }
...
int CU::ToVector(_out_ CVector* vector, _in_ int leftRows, _in_ int rowCursorInCU, _out_ int& deadRows, _in_ uint8* cuDelMask)
{
    int num = 0;

    if (!this->HasNullValue())
        num = this->ToVectorT<attlen, false, hasDeadRow>(vector, leftRows, rowCursorInCU, deadRows, cuDelMask);
    else
        num = this->ToVectorT<attlen, true, hasDeadRow>(vector, leftRows, rowCursorInCU, deadRows, cuDelMask);
    return num;
}

// enumerate possible instances
template int CU::ToVector<1, true>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<2, true>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<4, true>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<8, true>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<12, true>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<16, true>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<-1, true>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<-2, true>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<1, false>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<2, false>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<4, false>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<8, false>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<12, false>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<16, false>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<-1, false>(CVector*, int, int, int&, uint8*);
template int CU::ToVector<-2, false>(CVector*, int, int, int&, uint8*);
...
void CU::FreeSrcBuf()
{
    ...
    if (m_offset) {
        ...
        CStoreMemAlloc::Pfree(m_offset_ps, !m_inCUCache);
    }
    m_offset = NULL;
    m_offset_ps = NULL;
    m_offsetSize = 0;
